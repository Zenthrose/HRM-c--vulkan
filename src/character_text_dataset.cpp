...
CharacterTextDataset::CharacterTextDataset(const std::string& data_path, std::shared_ptr<UTF8Processor> utf8_processor)
    ...
    for (char32_t c = 32; c < 127; ++c) {  // Printable ASCII
...
Tensor CharacterTextDataset::encode_characters_to_tensor(const std::string& text) {
    std::vector<char32_t> codepoints = utf8_to_codepoints(text);
...
std::vector<char32_t> CharacterTextDataset::utf8_to_codepoints(const std::string& utf8_str) const {
    std::vector<char32_t> codepoints;
        ...
        if (byte < 0x80) {
            ...
            if (i + 1 < utf8_str.length()) {
                codepoint = ((byte & 0x1F) << 6) |
                           (static_cast<unsigned char>(utf8_str[i + 1]) & 0x3F);
                i += 2;
            } else {
...
std::string CharacterTextDataset::codepoints_to_utf8(const std::vector<char32_t>& codepoints) const {
    std::string utf8_str;
