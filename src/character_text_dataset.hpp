...
public:
     ...
     */
    const std::unordered_map<char32_t, int>& get_char_to_id() const { return char_to_id_; }

    /**
     * Get ID-to-character mapping
     * @return Const reference to ID mapping
     */
    const std::vector<char32_t>& get_id_to_char() const { return id_to_char_; }
...
private:
     ...
     */
    std::vector<char32_t> utf8_to_codepoints(const std::string& utf8_str) const;
     ...
     */
    std::string codepoints_to_utf8(const std::vector<char32_t>& codepoints) const;
