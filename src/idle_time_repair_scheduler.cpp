...
SystemIdleState IdleTimeRepairScheduler::get_current_idle_state() const {
    ...
    // Check user activity (simplified - in real implementation would check keyboard/mouse)
...
void IdleTimeRepairScheduler::idle_monitor_loop() {
    while (scheduler_running_) {
        ...
        // In a real implementation, this would monitor keyboard/mouse events
        last_user_activity_ = std::chrono::system_clock::now();
...
bool IdleTimeRepairScheduler::is_within_repair_window() const {
    auto now = std::chrono::system_clock::now();
    auto time_of_day = std::chrono::hours(now.time_since_epoch().count() % (24 * 3600) / 3600);

    for (const auto& window : repair_windows_) {
        if (time_of_day >= window.first && time_of_day <= window.second) {
            return true;
        }
    }
